...
Vec2D.avec_new = function(u, t) { // angular vec new
    return u.mulS(t);
}
Vec2D.avec_rand = function(min, max) {
    var x = Scalar.rand(-1, 1);
    var y = Scalar.rand(-1, 1);
    return new Vec2D(x, y, z).unit().mulS(Scalar.rand(min, max));
}
...
Mat2D.rmat_new = function(t) { // rotate mat new
    var c = Math.cos(t);
    var s = Math.sin(t);
...
/**
 * Draw2D: Draw point and line for 2D
 */
var Draw2D = {};
...
/**
 * Draw3D: Draw point and line for 3D
 */
var Draw3D = {};
...
function Obj2D(pos, vel, mass, size) {
    ...
    this.va = vel[1]; // 1D
    this.ml = mass;
    this.rt = Mat2D.rmat_new(this.pa);
